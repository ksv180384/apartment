...
namespace App\Http\Requests\Admin\LayoutType;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class UpdateLayoutTypeRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        ...
        return [
            'name' => [
                'required',
                'min:2',
                Rule::unique('layout_types', 'name')->ignore($propertyTypeId)
            ],
            'slug' => [
                'nullable',
                'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/',
                Rule::unique('layout_types', 'slug')->ignore($propertyTypeId)
            ],
