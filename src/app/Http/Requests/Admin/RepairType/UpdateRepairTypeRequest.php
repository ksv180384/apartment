...
namespace App\Http\Requests\Admin\RepairType;
...
class UpdateRepairTypeRequest extends FormRequest
{
    ...
    {
        $repairTypeId = $this->route('id');

        return [
            'name' => [
                'required',
                'min:2',
                Rule::unique('repair_types', 'name')->ignore($repairTypeId)
            ],
            'slug' => [
                'nullable',
                'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/',
                Rule::unique('repair_types', 'slug')->ignore($repairTypeId)
            ],
        ];
    }
