...
class UpdateBuildingClassRequest extends FormRequest
{
    /**
    ...
    {
        $buildingClassId = $this->route('id');

        return [
            'name' => [
                'required',
                'min:2',
                Rule::unique('building_classes', 'name')->ignore($buildingClassId)
            ],
            'slug' => [
                'nullable',
                'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/',
                Rule::unique('building_classes', 'slug')->ignore($buildingClassId)
            ],
        ];
    }
