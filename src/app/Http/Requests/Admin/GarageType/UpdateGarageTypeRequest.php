...
namespace App\Http\Requests\Admin\GarageType;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class UpdateGarageTypeRequest extends FormRequest
{
    /**
    ...
    {
        $garageTypesId = $this->route('id');

        return [
            'name' => [
                'required',
                'min:2',
                Rule::unique('garage_types', 'name')->ignore($garageTypesId)
            ],
            'slug' => [
                'nullable',
                'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/',
                Rule::unique('garage_types', 'slug')->ignore($garageTypesId)
            ],
        ];
    }
